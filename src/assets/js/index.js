...
class Splash {
    ...
    async startAnimation() {
        let splashes = [
            { "message": "Todo esta mejor despues de un vermut.", "author": "Vermucito" },
            { "message": "¿Cuantas formas sabes de preparar vermut?", "author": "Vermucito" },
            { "message": "Ella le dio nombre a esto, no se que pasara si ella ya no estuviera.", "author": "Vermucito" }
        ];
    ...
    async checkUpdate() {
        ...
        ipcRenderer.invoke('update-app').then().catch(err => {
            return this.shutdown(`Error al buscar atualizaciones :<br>${err.message}`);
        });
    ...
    async dowloadUpdate() {
        ...
        this.setStatus(`Actualización disonible !<br><div class="download-update">Descargar</div>`);
    ...
    shutdown(text) {
        this.setStatus(`${text}<br>Cerrar en 5s`);
        let i = 4;
        setInterval(() => {
            this.setStatus(`${text}<br>Cerrar en ${i--}s`);
